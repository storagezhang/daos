...
class DmSerialSmallTest(DataMoverTestBase):
    ...
    def setUp(self):
        ...
        super(DmSerialSmallTest, self).setUp()
            ...
            for dkey_idx in range(self.num_dkeys_per_obj):
                ...
                for akey_idx in range(self.num_akeys_array_per_dkey):
                    ...
                    for data_idx in range(num_extents):
                        ...
                        data = data_size * data_val
                        c_data.append([ctypes.create_string_buffer(data), data_size])
    ...
    def verify_dataset(self, cont):
        ...
        for obj_idx in range(self.num_objs):
            obj = DaosObj(cont.pool.context, cont.container, self.obj_list[obj_idx].c_oid)
            obj.open()
            ioreq = IORequest(cont.pool.context, cont.container, obj)
            for dkey_idx in range(self.num_dkeys_per_obj):
    ...
    def run_dm_serial_small(self, tool):
        ...
        # TODO create a function for this in the test base
        uuid_search = re.search(
            r"Successfully created container (.*-.*-.*-.*-.*)",
            result.stdout)
        if not uuid_search:
            self.fail("Failed to parse destination container uuid")
        cont2_uuid = uuid_search.group(1)
