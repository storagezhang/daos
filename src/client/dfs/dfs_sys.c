	      ...
	      dfs_sys_t **_dfs_sys)
{
	...
	if (rc != 0) {
		D_DEBUG(DB_ALL, "dfs_mount() failed (%d)\n", rc);
	...
	if (dfs_sys->use_hash) {
		...
		rc = d_hash_table_create(hash_feats, DFS_SYS_HASH_SIZE,
					 ...
					 &dfs_sys->dfs_hash);
		if (rc != 0) {
			D_DEBUG(DB_ALL, "d_hash_table_create() failed: "
				DF_RC"\n", DP_RC(rc));
...
dfs_sys_umount(dfs_sys_t *dfs_sys)
{
	...
	if (dfs_sys->use_hash) {
		...
		if (rc != 0) {
			D_DEBUG(DB_ALL, "d_hash_table_destroy() failed: "
				DF_RC"\n", DP_RC(rc));
			D_GOTO(err_hash, rc = daos_der2errno(rc));
		}
	}
	...
	if (rc != 0)
		D_DEBUG(DB_ALL, "dfs_umount() failed (%d)\n", rc);
...
dfs_sys_access(dfs_sys_t *dfs_sys, const char *path, int mask, int flags)
{
	...
	if (flags & AT_SYMLINK_NOFOLLOW) {
		...
		rc = dfs_lookup_rel(dfs_sys->dfs, sys_path.parent,
				    ...
				    &mode, NULL);
		if (rc != 0) {
			D_DEBUG(DB_ALL, "dfs_lookup_rel() %s failed: (%d)\n",
				sys_path.name, rc);
	     ...
	     int flags)
{
	...
	if (flags & O_NOFOLLOW) {
		rc = dfs_lookup_rel(dfs_sys->dfs, sys_path.parent,
				    sys_path.name, O_RDWR, &obj, NULL, buf);
		if (rc != 0) {
			D_DEBUG(DB_ALL, "dfs_lookup_rel() %s failed: (%d)\n",
				sys_path.name, rc);
		}
	      ...
	      daos_oclass_id_t cid, daos_size_t chunk_size)
{
	...
	if (rc != 0) {
		D_DEBUG(DB_ALL, "dfs_open() %s failed: (%d)\n",
			sys_path.name, rc);
		D_GOTO(out_free_path, rc);
	}
...
dfs_sys_read(dfs_sys_t *dfs_sys, dfs_obj_t *obj, void *buf, daos_size_t *size,
	     daos_off_t offset, daos_event_t *ev)
{
	d_iov_t		iov;
	d_sg_list_t	sgl;
	...
	return dfs_read(dfs_sys->dfs, obj, &sgl, offset, size, ev);
}

int
dfs_sys_write(dfs_sys_t *dfs_sys, dfs_obj_t *obj, const void *buf,
	      daos_size_t size, daos_off_t offset, daos_event_t *ev)
{
	d_iov_t		iov;
	d_sg_list_t	sgl;
	...
	return dfs_write(dfs_sys->dfs, obj, &sgl, offset, ev);
}
...
dfs_sys_ftruncate(dfs_sys_t *dfs_sys, dfs_obj_t *obj, daos_off_t length)
{
	...
	return dfs_punch(dfs_sys->dfs, obj, length, DFS_MAX_FSIZE);
}
...
dfs_sys_opendir(dfs_sys_t *dfs_sys, const char *dir, DIR **_dirp)
{
	...
	if (sys_path.dir_name == NULL) {
		rc = dfs_dup(dfs_sys->dfs, sys_path.parent, O_RDWR,
			     &sys_dir->obj);
		if (rc) {
			D_DEBUG(DB_ALL, "dfs_dup() %s failed: (%d)\n",
				sys_path.name, rc);
			D_GOTO(out_free_dir, rc);
		}
		D_GOTO(out, rc);
