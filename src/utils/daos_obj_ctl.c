...
int rc;
  ...
  uuid_unparse_lower(ap->p_uuid, uuid_str);
	fprintf(stdout, "Pool %s\n", uuid_str);

	uuid_generate(ctl_ctx.tsc_cont_uuid);

	fprintf(stdout, "DAOS system name: %s\n", ap->sysname);
	...
	*/
	...
	/* ctl_ctx.sysname = ap->sysname; ya no existe?? */

	fprintf(stdout, "obj_ctl_shell() - B\n");
	...
	fprintf(stdout, "obj_ctl_shell() - C\n");

	rc = dts_cmd_parser(ctl_ops, "$ > ", ctl_cmd_run);

	fprintf(stdout, "obj_ctl_shell() - D\n");

	if (rc) {
		ctx_fini(&ctl_ctx);
	  fprintf(stdout, "obj_ctl_shell() - E\n");
  }
	return rc;
}
...
obj_ctl_shell(struct cmd_args_s *ap)
{
	...
	fprintf(stdout, "obj_ctl_shell() - A\n");
	...
	case SH_DAOS:
		...
		break;
	case SH_VOS:
		fprintf(stdout, "vos no yet implemented\n");
		rc = -DER_NOSYS;
		break;
	default:
